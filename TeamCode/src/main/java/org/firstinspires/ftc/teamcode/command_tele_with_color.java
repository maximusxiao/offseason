...
import org.firstinspires.ftc.teamcode.Subsystems.color_sensor;
...
@TeleOp(name = "Command OpMode With Color detection")
public class command_tele_with_color extends PedroOpMode {
    public command_tele_with_color() {
        super(lift.INSTANCE, extendo.INSTANCE, claw.INSTANCE, pitch.INSTANCE, intake_arm.INSTANCE, lift_arm.INSTANCE, pto.INSTANCE, stilts.INSTANCE, wrist.INSTANCE, intake.INSTANCE, color_sensor.INSTANCE);
    }
        ...
        gamepadManager.getGamepad2().getRightTrigger().setStateChangeCommand(
                value -> new ParallelGroup(
                        ...
                        new BlockingConditionalCommand(
                                () -> color_sensor.INSTANCE.check_color() == true,
                                () -> new Delay(0.0),
                                () -> new ParallelGroup(
                                        intake_arm.INSTANCE.intake(),
                                        pitch.INSTANCE.reject(),
